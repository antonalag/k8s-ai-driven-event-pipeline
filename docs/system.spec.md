...
12. **Audit Log & Historical Queries (Phase 19):** Paginated read-path for resolved analysis records (REMEDIATED/DISMISSED) with a CQRS-lite `AuditLogQueryPort`, `OpenSearchAuditLogQueryAdapter` using native Spring Data pagination, REST endpoint `GET /api/v1/analyses/history`, `modelUsed` LLM tracking across all adapters, and a dedicated "Audit Log" tab in the Observability UI with TanStack Query integration and status-driven color badges.
...
### ✅ Phase 19 — Audit Log & Historical Queries (Completed)
**Architecture:** Introduces a dedicated paginated read-path for resolved (REMEDIATED/DISMISSED) AI analysis records, enabling operators to review historical remediation and dismissal decisions. The design follows CQRS-lite separation — a new `AuditLogQueryPort` distinct from the active dashboard's query port. Additionally, a cross-cutting enhancement adds `modelUsed` tracking to the `AiAnalysis` domain record, propagated from the AI provider (Ollama/BYOK) configuration through persistence and all read models.

**Components:**
- **Domain:** `AuditLogQueryPort` (inbound port), `AuditLogPage` and `AuditLogEntry` value objects (pure domain)
- **Infrastructure Persistence:** `OpenSearchAuditLogQueryAdapter` with `findByStatusIn` query, native `Pageable` pagination, `resolvedAt DESC` sort
- **Infrastructure Web:** `AuditLogController` (`GET /api/v1/analyses/history`), `AuditLogValidationException`, RFC 7807 error handling, silent size cap at 100
- **Model Tracking:** `AiAnalysis.modelUsed` field with backward-compatible constructors, Ollama/BYOK/Resilient adapter propagation
- **Frontend:** "Audit Log" sidebar tab, `useAuditLogHistory` TanStack Query hook (30s staleTime), `AuditLogView` table with status badges

- [x] **Milestone 47 (Completed):** Domain Layer — `AuditLogEntry` record, `AuditLogPage` record, `AuditLogQueryPort` interface, `AiAnalysis` extended with `modelUsed` field and backward-compatible constructors (5-arg, 7-arg). Zero Spring imports.
- [x] **Milestone 48 (Completed):** Infrastructure Persistence — `AiAnalysisDocument.modelUsed` Keyword field, `SpringDataAiAnalysisRepository.findByStatusIn` query method, `OpenSearchAuditLogQueryAdapter` with validation, pagination, and `modelUsed` defaulting.
- [x] **Milestone 49 (Completed):** Infrastructure Web — `HistoryResponseDto` and `AuditLogHistoryResponse` DTOs, `AuditLogValidationException` with RFC 7807 handler, `AuditLogController` with size capping and parameter validation.
- [x] **Milestone 50 (Completed):** Model Tracking — Ollama adapter passes `platform.ai.ollama.model`, BYOK adapter passes `platform.ai.byok.model`, Resilient fallback uses `"unknown"`. `AiAnalysisView` and `AiAnalysisResponse` extended with `modelUsed`.
- [x] **Milestone 51 (Completed):** Testing & Quality — 13 correctness properties (jqwik backend: status filter, sort, pagination, validation, size capping, DTO completeness, modelUsed propagation/defaults; fast-check frontend: badge color, pagination controls, error rendering, table columns). MockMvc controller tests.
- [x] **Milestone 52 (Completed):** Frontend Integration — TypeScript types (`ResolvedStatus`, `HistoryResponseDto`, `AuditLogHistoryResponse`), `useAuditLogHistory` hook, `AuditLogView` component with table, pagination, skeleton loading, empty state, dismissible error notification, and sidebar navigation.